...
def messages():
    ...
    recent_messages = []
    sender_messages = Messages.query.filter_by(sender_username = current_user.username).order_by(Messages.time_sent.desc()).all()
    recipient_messages =  Messages.query.filter_by(recipient_username = current_user.username).order_by(Messages.time_sent.desc()).all()
    ...
    return render_template('messages.html', recent_messages=recent_messages, current_user=current_user)
...
def new_message():
    ...
    return render_template('new_message.html', usernames=sorted(usernames), sender=current_user.username)

@router.post('/new/<recipient>')
def send_new_message(recipient):
    data = request.json

    sender_checker = Messages.query.with_entities(Messages.sender_username, Messages.recipient_username, Messages.conversation_id).filter_by(sender_username = data['sender'], recipient_username = data['recipient']).all()
    recipient_checker =  Messages.query.with_entities(Messages.sender_username, Messages.recipient_username, Messages.conversation_id).filter_by(sender_username = data['recipient'], recipient_username = data['sender']).all()
    ...
    if (db.session.query(func.max(Messages.conversation_id)).all()[0][0] == None):
        ...
        new_message.conversation_id = int(db.session.query(func.max(Messages.conversation_id)).all()[0][0]) + 1
...
def get_conversation(conversation_id):
    ...
    return render_template('conversation.html', recipient=recipient, messages=messages, current_user=current_user, conversation_id=conversation_id, sender=current_user.username)

@router.post("/conversation/<conversation_id>")
def get_conversation_message(conversation_id):
    data = request.json
    new_message = Messages()
    new_message.conversation_id = data['conversation_id']
    new_message.recipient_username = data['recipient']
    new_message.sender_username = data['sender']
    new_message.message = data['message']
    new_message.time_sent = datetime.now()
